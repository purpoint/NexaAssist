...
def test_no_migration_drops_an_enum_check_constraint() -> None:
    """Autogenerate repeatedly proposes dropping these constraints.

    Any constraint generated by ``Enum(create_constraint=True)`` -- rather than
    declared in ``__table_args__`` -- goes unmatched during comparison, so every
    autogenerated draft offers to delete it. First seen on
    ``ck_tickets_status_valid``; it recurred on ``ck_review_items_*`` the moment
    a second enum column existed, so the guard covers the shape, not one name.

    Catching it here is cheaper than noticing in production that any string is
    now a valid status.
    """
    guarded = ("_status_valid", "_role_valid")
    import ast
    ...
    for path in VERSIONS.glob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            for suffix in guarded:
                assert suffix not in rendered, f"{path.name} drops {rendered}"
